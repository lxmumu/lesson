        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .= ' <li><a href="' . build_uri($type, $args, $val['cat_name']) . '">' . htmlspecialchars($val['cat_name']) . '</a>'.'</li>';
                }
            }
        }
        /* 处理无分类的 */
...
//     var_dump(get_navigator($ctype, $catlist)['top'][0]);exit;
    $smarty->assign('navigator_list', get_navigator($ctype, $catlist));  //自定义导航栏
    // $smarty->assign('cart_data', get_navigator($ctype, $catlist)['top'][0]);
