 ...
 * @return  array
 */
function get_promote_goods($cats = '',$type='promote',$num=0)
{
    $time = gmtime();
    $order_type = $GLOBALS['_CFG']['recommend_order'];
    $str_type=" AND g.is_promote = 1 AND promote_start_date <= '$time' AND promote_end_date >= '$time' ";
    if($type=='first'){
        $str_type=" AND g.is_first = 1 AND first_start_date <= '$time' AND first_end_date >= '$time' ";
    }
    /* 取得促销lbi的数量限制 */
    if(!$num>0){
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "g.is_best, g.is_new, g.is_hot, g.is_promote, RAND() AS rnd,g.goods_desc " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 '.$str_type;
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        $goods[$idx]['goods_desc']   = mb_substr(strip_tags($row['goods_desc']),0,100);
        $goods[$idx]['goods_style_name']   = add_style($row['goods_name'],$row['goods_name_style']);
...
{
    $sql = 'SELECT img_id, img_url, thumb_url,thumb_middle_url, img_desc' .
    ...
    foreach($row as $key => $gallery_img)
    {
        ...
        $row[$key]['thumb_middle_url'] = get_image_path($goods_id, $gallery_img['thumb_middle_url'], true);
    }
//    echo '<pre>';
//    var_dump($row);
//    echo '</pre>';
