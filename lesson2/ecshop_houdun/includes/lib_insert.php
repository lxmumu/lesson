...
/*            <li>
                                ...
                                <div class="g-info">
                                        <strong>21%会买：</strong>
                                        <a href="###" title="好手机" target="_blank">商品名</a>
                                </div>
                                <div class="g-price">
                                        ￥3200.00
                                </div>
            ...
            $str.='<li>
                                <div class="g-img">
                                        <a href="'.$goods['url'].'"><img src="'.$goods['goods_thumb'].'" alt="'.strip_tags($goods['goods_name']).'" /></a>
                                        <b></b>
                                </div>
                                <div class="g-info">
                                        <a href="###" title="'.strip_tags($goods['goods_name']).'" target="_blank">'.$goods['short_name'].'</a>
                                </div>
                                <div class="g-price">
                                        '.$goods['shop_price'].'
                                </div>
        ...
        $str.='<li><a href="#" onclick="return clear_history();">清除浏览记录</a></li>';
...
function insert_catgoods($arr){
    if(!isset($arr['id'])){
        exit('<script type="text/javascript">alert("insert标签name为catgoods时必须设置id属性");</script>');
    }
    $need_cache = $GLOBALS['smarty']->caching;
    $GLOBALS['smarty']->caching = false;
    $categories=get_categories_tree($arr['id']);
    $children = get_children($arr['id']);
    $default_sort_order_method = $_CFG['sort_order_method'] == '0' ? 'DESC' : 'ASC';
    $default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'last_update');

    $goodslist = category_get_goods($children, 0, 0, 0, '', 8, 1, $default_sort_order_type, $default_sort_order_method);

    $GLOBALS['smarty']->assign('goodslist', $goodslist);
    $GLOBALS['smarty']->assign('categories', $categories);
    $output = $GLOBALS['smarty']->fetch('library/index_cate_goods.lbi');
