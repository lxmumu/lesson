...
//                新加
                $thumb_middle_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key], $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                $thumb_middle_url = is_string($thumb_middle_url) ? $thumb_middle_url : '';
                //                /新加
            }
            ...
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url,thumb_middle_url, img_original) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$thumb_middle_url', '$img_original')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
        ...
        elseif (!empty($image_urls[$key]) && ($image_urls[$key] != $GLOBALS['_LANG']['img_file']) && ($image_urls[$key] != 'http://') && copy(trim($image_urls[$key]), ROOT_PATH . 'temp/' . basename($image_urls[$key])))
        {
            ...
            if ($proc_thumb)
            {
                ...
                $thumb_middle_url = $GLOBALS['image']->make_thumb($down_img, $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                $thumb_middle_url = is_string($thumb_middle_url) ? $thumb_middle_url : '';
            }
